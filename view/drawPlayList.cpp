...
playlist::playlist() : songFont(22, 8, 0x333333, "微软雅黑") {
...
play_list_controller::play_list_controller() : tabFont(22, 8, 0x333333, "微软雅黑") {
    ...
    add_tab.tab_button = new button_txt(
        ...
        RGB(130, 185, 255), button_style::roundrect, txt(22, 8, 0x000000, "微软雅黑")
    );
...
void play_list_controller::add_playlist_tab() {
    ...
    std::string name = "歌单" + std::to_string(tabs.size());
    button_txt* new_btn = new button_txt(
...
void play_list_controller::handle_wheel(int wheel_move, int mouse_x, int mouse_y) {
    ...
    if (mouse_y >= ctrl_y && mouse_y <= ctrl_y + ctrl_h) {
        ...
        if (speed == 0) return;
        // 边界限制 (Clamping)
        if (speed > 0 && first_x + speed > view_left) speed = view_left - first_x;     // 左边界限制
        else if (speed < 0 && (last_x + last_w) + speed < view_right) speed = view_right - (last_x + last_w); // 右边界限制
        // 应用移动
        if (speed != 0) {
