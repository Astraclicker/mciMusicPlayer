...
void load_file(std::string music_address, std::vector<Song> &songs_list) {
    ...
    for (const auto &entry: fs::directory_iterator(music_address)) {
        ...
        if (back == name1 || back == name2) {
            std::string name = temp.substr(music_address.size() + 1,
                                           temp.size() - music_address.size() - back.size() - 2);
            char length[64];
            mciSendString("close myaudio", NULL, 0, NULL);
            std::string cmd = "open \"" + music_address + "/" + STRtoANSI(name) + "." + STRtoANSI(back) +
                              "\"  alias myaudio";
            mciSendString(cmd.c_str(), NULL, 0, NULL);
            mciSendString("status myaudio length", length, sizeof(length), NULL);
            int total_ms = atoi(length);
            songs_list.push_back({
                n, STRtoANSI(name), STRtoANSI(music_address),
                music_address + "/" + STRtoANSI(name) + "." + STRtoANSI(back), total_ms
            });
            n++;
        }
    }
    ...
    for (auto i: songs_list) {
        std::cout << i.song_index << "||" << i.song_name << "||" << i.song_root << "||" << i.song_address << "||" << i.
                song_time << std::endl;
    }
}
