...
// 构造函数
button::button(int x, int y, int length, int width, int color, button_style b_style)
    : x(x), y(y), length(length), width(width), color(color), b_style(b_style), is_visible(true) {
    // 基础属性初始化完成
}

// 通用的矩形区域检测 (AABB)
bool button::checkButton(int mouse_x, int mouse_y) {
    // 如果按钮不可见，直接返回 false
...
button_txt::button_txt(int x, int y, int w, int h, int color, button_style style,
    ...
    : button(x, y, h, w, color, style), // 注意：基类构造参数顺序是 length(高), width(宽)，需根据您的定义对应
...
button_img::button_img(int x, int y, int length, int width,
    ...
    : button(x, y, length, width, 0, button_style::roundrect), // 图片按钮通常不需要背景色，给0
...
bool button_img::checkButton(int mouse_x, int mouse_y) {
    ...
    // 直接调用基类的检测，省性能
    if (!button::checkButton(mouse_x, mouse_y)) {
        return false;
    }
